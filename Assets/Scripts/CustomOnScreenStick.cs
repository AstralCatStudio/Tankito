...
namespace Tankito.Mobile
{
    public class CustomOnScreenStick : OnScreenControl, IPointerDownHandler, IPointerUpHandler, IDragHandler
    {
        ...
        private void Start()
        {
            ...
            image.color = new Color(1, 1, 1, 0.1f); // MOD
        ...
        private void BeginInteraction(Vector2 pointerPosition, Camera uiCamera)
        {
            ...
            if (canvasRect == null)
            {
                Debug.LogError("CustomOnScreenStick needs to be attached as a child to a UI Canvas to function properly.");
                return;
            }
        ...
        [SerializeField] public Sprite m_circleSprite;
...
#if UNITY_EDITOR
        [CustomEditor(typeof(CustomOnScreenStick))]
        internal class OnScreenStickEditor : UnityEditor.Editor
        {
            ...
            public void OnEnable()
            {
                ...
                m_UseIsolatedInputActions = serializedObject.FindProperty(nameof(CustomOnScreenStick.m_UseIsolatedInputActions));

                m_Behaviour = serializedObject.FindProperty(nameof(CustomOnScreenStick.m_Behaviour));
                m_ControlPathInternal = serializedObject.FindProperty(nameof(CustomOnScreenStick.m_ControlPath));
                m_MovementRange = serializedObject.FindProperty(nameof(CustomOnScreenStick.m_MovementRange));
                m_DynamicOriginRange = serializedObject.FindProperty(nameof(CustomOnScreenStick.m_DynamicOriginRange));
                m_PointerDownAction = serializedObject.FindProperty(nameof(CustomOnScreenStick.m_PointerDownAction));
                m_PointerMoveAction = serializedObject.FindProperty(nameof(CustomOnScreenStick.m_PointerMoveAction));
            }
            ...
            public override void OnInspectorGUI()
            {
                ...
                m_ShowDynamicOriginOptions.target = ((CustomOnScreenStick)target).behaviour ==
                ...
                if (EditorGUILayout.BeginFadeGroup(m_ShowDynamicOriginOptions.faded))
                {
                    ...
                    {
                        ((CustomOnScreenStick)target).UpdateDynamicOriginClickableArea();
                    }
                    EditorGUI.indentLevel--;
                }
